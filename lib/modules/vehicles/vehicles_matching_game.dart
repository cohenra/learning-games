...
class VehiclesMatchingGame extends StatefulWidget {
  const VehiclesMatchingGame({super.key});

  @override
  State<VehiclesMatchingGame> createState() => _VehiclesMatchingGameState();
}

class _VehiclesMatchingGameState extends State<VehiclesMatchingGame> {
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    IconButton(icon: Icon(_isHebrew ? Icons.arrow_forward : Icons.arrow_back, color: Colors.blue.shade700, size: 32), onPressed: () => Navigator.pop(context)),
                    Expanded(child: Text(_isHebrew ? 'משחק התאמה' : 'Matching Game', style: TextStyle(fontSize: responsive.titleSize, fontWeight: FontWeight.bold, color: Colors.blue.shade700), textAlign: TextAlign.center)),
                    SizedBox(width: 48),
                  ],
                ),
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.blue.withOpacity(0.2), blurRadius: 8, offset: const Offset(0, 4))]),
                child: Column(
                  children: [
                    Text(_isHebrew ? 'איפה ה...' : 'Where is the...', style: TextStyle(fontSize: responsive.fontSize(22), color: Colors.grey.shade700)),
                    const SizedBox(height: 8),
                    Text(_isHebrew ? _correctItem['nameHe']! : _correctItem['nameEn']!, style: TextStyle(fontSize: responsive.fontSize(32), fontWeight: FontWeight.bold, color: Colors.blue.shade700)),
                  ],
                ),
              ),
              SizedBox(height: responsive.spacing(20)),
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                Flexible(flex: 1, child: Text(_isHebrew ? option['nameHe']! : option['nameEn']!, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.blue.shade700), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis)),
              ...
              Padding(padding: const EdgeInsets.all(16), child: KidButton(text: _isHebrew ? 'שמע שוב את השאלה 🔊' : 'Hear Question Again 🔊', icon: Icons.volume_up, onPressed: _speakQuestion, color: Colors.blue, height: 60)),
            ],
          ),
        ),
      ),
