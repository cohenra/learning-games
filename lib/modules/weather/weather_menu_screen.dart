...
class WeatherMenuScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isHebrew ? 'מזג אוויר ☀️🌧️' : 'Weather ☀️🌧️'),
        centerTitle: true,
                  ...
                  child: IntrinsicHeight(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(isHebrew ? 'מזג אוויר' : 'Weather', style: TextStyle(fontSize: responsive.titleSize * 0.9, fontWeight: FontWeight.bold, color: Colors.teal.shade700)),
                        const SizedBox(height: 8),
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: Text(isHebrew ? 'למדו על מזג אוויר שונה' : 'Learn about different weather', style: TextStyle(fontSize: responsive.subtitleSize * 0.85, color: Colors.grey.shade700), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
                        ),
  ...
  List<Map<String, dynamic>> _getButtons(BuildContext context, bool isHebrew) {
    return [
      {'text': isHebrew ? 'למידה 📚' : 'Learning 📚', 'icon': Icons.school, 'color': Colors.teal.shade500, 'onPressed': () => Navigator.push(context, MaterialPageRoute(builder: (context) => const WeatherLearningScreen()))},
      {'text': isHebrew ? 'משחק התאמה 🎮' : 'Matching Game 🎮', 'icon': Icons.gamepad, 'color': Colors.cyan.shade500, 'onPressed': () => Navigator.push(context, MaterialPageRoute(builder: (context) => const WeatherMatchingGame()))},
    ];
  }
}
