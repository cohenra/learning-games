...
class WeatherMatchingGame extends StatefulWidget {
  const WeatherMatchingGame({super.key});

  @override
  State<WeatherMatchingGame> createState() => _WeatherMatchingGameState();
}

class _WeatherMatchingGameState extends State<WeatherMatchingGame> {
  ...
  final List<Map<String, String>> _allItems = [
    ...
    {'emoji': '⛈️', 'nameHe': 'סוער', 'nameEn': 'Stormy'},
    {'emoji': '🌨️', 'nameHe': 'שלג', 'nameEn': 'Snowy'},
    {'emoji': '☁️', 'nameHe': 'מעונן', 'nameEn': 'Cloudy'},
  ];
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text(_isHebrew ? 'משחק התאמה' : 'Matching Game', style: TextStyle(fontSize: responsive.titleSize, fontWeight: FontWeight.bold, color: Colors.teal.shade700), textAlign: TextAlign.center)),
                    SizedBox(width: 48),
                  ],
                ),
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.teal.withOpacity(0.2), blurRadius: 8, offset: const Offset(0, 4))]),
                child: Column(
                  children: [
                    Text(_isHebrew ? 'איפה ה...' : 'Where is the...', style: TextStyle(fontSize: responsive.fontSize(22), color: Colors.grey.shade700)),
                    const SizedBox(height: 8),
                    Text(_isHebrew ? _correctItem['nameHe']! : _correctItem['nameEn']!, style: TextStyle(fontSize: responsive.fontSize(32), fontWeight: FontWeight.bold, color: Colors.teal.shade700)),
                  ],
                ),
              ),
              SizedBox(height: responsive.spacing(20)),
                  ...
                  builder: (context, constraints) {
                    ...
                    return GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        Color borderColor = Colors.teal.shade200;
              ...
              Padding(padding: const EdgeInsets.all(16), child: KidButton(text: _isHebrew ? 'שמע שוב את השאלה 🔊' : 'Hear Question Again 🔊', icon: Icons.volume_up, onPressed: _speakQuestion, color: Colors.teal, height: 60)),
            ],
          ),
        ),
      ),
